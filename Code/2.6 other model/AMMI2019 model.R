...
earPhe2019_2<-earPhe2019%>%
  group_by(Genotype,Environment,IndexRep)%>%
...
model<- AMMI(ENV=earPhe2019_2$Environment,
             GEN=earPhe2019_2$EventID,
             REP=1,
             Y=earPhe2019_2$M,console=F,PC=F)
write_rds(model,"2019AMMI_reslut.rds")
##查看方差分析
model$ANOVA
...
write.csv(Idx2,"2019AMMI_index.csv")
